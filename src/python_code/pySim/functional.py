...
class SimFunctionForce(autograd.Function):
    ...
    def forward(
        ...
        argV = np.float64(v.contiguous().detach().cpu().numpy())
        argA = np.float64(a.contiguous().detach().cpu().numpy())
        argF = np.float64(f.contiguous().detach().cpu().numpy())
        cppSim.stepForceNN(ctx.pastRecord.stepIdx + 1, argX, argV, argA, argF)
        ...
        ctx.save_for_backward(x, v, a, f, x_next, v_next)
    ...
    def backward(
        ...
        x, v, a, f, x_next, v_next = ctx.saved_tensors
        ...
        if isLast:
            backRecord = cppSim.stepForceBackwardNN(
                ctx.helper.taskInfo,
                np.zeros_like(dL_dxnew_np),
                np.zeros_like(dL_dvnew_np),
                ctx.newRecord,
        ...
        dL_df = torch.as_tensor(backRecord.dL_dfext)

        return dL_dx, dL_dv, dL_da, dL_df, None, None