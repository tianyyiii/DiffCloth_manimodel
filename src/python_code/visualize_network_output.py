...
def render_record(sim, kp_idx=None, curves=None, title="default"):
    renderer = WireframeRenderer(backend="pyglet", title=title)
...
def render_record_to_gif(sim, kp_idx=None, curves=None, title="default", gif_path="default.gif"):
    renderer = WireframeRenderer(backend="pyglet", title=title)

    with open('camera_state.pkl', 'rb') as f:
        loaded_dict = pickle.load(f)
        renderer.set_cam_state(loaded_dict)
    ...
    # renderer.show()
    # renderer.run()
    renderer.render_gif(gif_path)


def main(mesh_path="../../python_code/DLG_Dress032_1.obj", result_path="/home/zzccppp/result", gif_path="/home/zzccppp/result/gif"):
    ...
    config = CONFIG.copy()
    config['fabric']['name'] = "objs/DLG/DLG_Dress032_1.obj"
    score = []
    start_time = time.time()
    for t in range(10):
        for trial in range(contact_force.shape[1]):
            attached_points = [contact_point[t, trial, 0],
                               contact_point[t, trial, 1]]
            config['scene']['customAttachmentVertexIdx'] = [
                (0.0, [contact_point[t, trial, 0], contact_point[t, trial, 1]])]
            ...
            helper_init = diffcloth.makeOptimizeHelperWithSim(
                "wear_hat", sim_init)
            pysim_init = pySim(sim_init, helper_init, True)
            target_state = state_data[init_step]["target_state"][(
                t % 440) % 20, :, :].flatten()

            min_loss = np.inf
            for i in range(30):
                a_delta = 0.01 * contact_force[t, trial, :, :].flatten(
                ) / np.linalg.norm(contact_force[t, trial, :, :].flatten())
                ...
                if loss < min_loss:
                    min_loss = loss
            print("---------------- RENDER SIMINIT ----------------")
            render_record_to_gif(sim_init, kp_idx=attached_points, title="sim_init", gif_path=gif_path + "/" + str(t) + "_" + str(trial) + "_init.gif")

            gt_point = state_data[init_step]["attached_point"][(
                t % 440) % 20, :]
            gt_target = state_data[init_step]["attached_point_target"][(
                t % 440) % 20, :, :]

            attached_points = [gt_point[0], gt_point[1]]
            config['scene']['customAttachmentVertexIdx'] = [
                (0.0, [gt_point[0], gt_point[1]])]
            ...
            print("---------------- RENDER SIMGT ----------------")
            render_record_to_gif(sim_gt, kp_idx=attached_points, title="sim_gt", gif_path=gif_path + "/" + str(t) + "_" + str(trial) + "_gt.gif")
...
if __name__ == '__main__':
    main(mesh_path="/home/zzccppp/Desktop/result/DLG_Dress032_1.obj",
         result_path="/home/zzccppp/Desktop/result", gif_path="/home/zzccppp/Desktop/result/gif")
