...
def apply_gradient_color(mesh, indices, max_distance=0.2, transparency=0.5):
    ...
    for i, dist in enumerate(normalized_distances):
        if dist < 1:
            colors[i, :] = np.array([1.0, 0., 0., 0.5])
        else:
            colors[i, :] = np.array([0.5, 0.5, 0.5, 0.5])
        #colors[i, :3] = (1 - dist) * np.array([1.0, 0, 0]) + dist * np.array([0.7, 0.7, 0.7])
...
def apply_arrow(mesh, attached_points, directions):
    ...
    for i, attached_point in enumerate(attached_points):
        ...
        colors = np.ones((len(arrow.vertices), 4)) * np.array([0.0, 0.0, 0.5, 0.9]) 
...
if __name__ == "__main__":
    base_dir = "/home/ubuntu/middle/ManiModelData/ClothesNetM/Trousers_0"
    cloth_dir = "/home/ubuntu/ManiModelData/ClothesNetM/Trousers"
    num = 30
    ...
    for top in tops:
        ...
        for path in paths:
            ...
            if stage_length > 6:
                ...
                mesh0 = apply_gradient_color(mesh0, points0)
                ...
                mesh1 = apply_gradient_color(mesh1, points1)
                ...
                mesh2 = apply_gradient_color(mesh2, points1)
