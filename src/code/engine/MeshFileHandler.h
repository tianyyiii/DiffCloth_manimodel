    ...
    static std::vector<Vec3d> loadPosFile_txt(std::string fileName) {
      ...
      if (myfile.is_open())
      {
        while ( getline (myfile,line) )
        {
          ...
          pos_i[1] = std::atof(line.substr(firstSpace+1, secondSpace-firstSpace-1).c_str());
          pos_i[2] = std::atof(line.substr(secondSpace+1, line.length()-secondSpace-1).c_str());
...
//        Triangle tri;
    ...
    static void saveOBJFile(std::string filename, VecXd x_n, std::vector<Vec3i> &triangles) {
      ...
      for (std::size_t i = 0; i < triangles.size(); ++i) {
        ...
        os << "f " << idx[0] + 1 << " " << idx[1] + 1 << " " << idx[2] + 1 << std::endl;
      }
...
//      std::printf("success!\n");
    ...
    static void saveOBJFile(std::string filename, std::vector<Vec3d> x_n, std::vector<Vec3i> &triangles) {
      checkFolderExistsAndCreate(OUTPUT_PARENT_FOLDER);
      std::ofstream os(OUTPUT_PARENT_FOLDER + filename + ".obj");
...
//      std::printf("success!\n");
    ...
    static void saveOBJFile(std::string filename, VecXd x_n, std::vector<Triangle> &triangles) {
      checkFolderExistsAndCreate(OUTPUT_PARENT_FOLDER);
      std::ofstream os(OUTPUT_PARENT_FOLDER + filename + ".obj");
...
//      std::printf("success!\n");
    ...
    static void saveOBJFile(std::string filename, VecXd x_n, VecXd normals, std::vector<Triangle> &triangles) {
      checkFolderExistsAndCreate(OUTPUT_PARENT_FOLDER);
      std::ofstream os(OUTPUT_PARENT_FOLDER + filename + ".obj");


      if (!os) {
        std::cerr << "file not exist:" << OUTPUT_PARENT_FOLDER + filename + ".obj" << std::endl;
        return;
      }
...
//      std::printf("success!\n");
    ...
    static void exportMeshPos(VecXd& x, std::string fileName) {
      ...
      std::printf("begin to save current mesh position to: %s....", fNameStream.str().c_str());
      myfile.open(fNameStream.str());

      int precision = 3;
      for ( int i = 0; i < x.rows() / 3; i++) {
        Vec3d pos = x.segment(i * 3, 3);
        myfile << std::fixed << std::setprecision(precision) << pos[0] << " " << pos[1] << " " << pos[2] << "\n";
      }
