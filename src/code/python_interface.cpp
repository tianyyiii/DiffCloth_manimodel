...
Simulation* makeSimFromConfig(py::dict config, bool runBackward = true)
{
    Simulation* sim = nullptr;

    auto fabricDict = config["fabric"].cast<py::dict>();
    auto sceneDict = config["scene"].cast<py::dict>();

    std::cout << static_cast<Orientation>(sceneDict["orientation"].cast<int>()) << std::endl;

    Simulation::FabricConfiguration farbic_config = {
        .clothDimX = fabricDict["clothDimX"].cast<double>(),
        .clothDimY = fabricDict["clothDimY"].cast<double>(),
        .k_stiff_stretching = fabricDict["k_stiff_stretching"].cast<double>(),
        .k_stiff_bending = fabricDict["k_stiff_bending"].cast<double>(),
        .gridNumX = fabricDict["gridNumX"].cast<int>(),
        .gridNumY = fabricDict["gridNumY"].cast<int>(),
        .density = fabricDict["density"].cast<double>(),
        .keepOriginalScalePoint = fabricDict["keepOriginalScalePoint"].cast<bool>(),
        .isModel = fabricDict["isModel"].cast<bool>(),
        .custominitPos = fabricDict["custominitPos"].cast<bool>(),
        .fabricIdx = fabricDict["fabricIdx"].cast<int>(),
        .color = fabricDict["color"].cast<Vec3d>(),
        .name = fabricDict["name"].cast<std::string>(),
    };

    Simulation::SceneConfiguration scene_config = {
        .fabric = farbic_config,
        .orientation = static_cast<Orientation>(sceneDict["orientation"].cast<int>()),
        .attachmentPoints = static_cast<AttachmentConfigs>(sceneDict["attachmentPoints"].cast<int>()),
        .customAttachmentVertexIdx = sceneDict["customAttachmentVertexIdx"].cast<std::vector<std::pair<double, std::vector<int>>>>(),
        .trajectory = static_cast<TrajectoryConfigs>(sceneDict["trajectory"].cast<int>()),
        .primitiveConfig = static_cast<PrimitiveConfiguration>(sceneDict["primitiveConfig"].cast<int>()),
        .windConfig = static_cast<WindConfig>(sceneDict["windConfig"].cast<int>()),
        .camPos = sceneDict["camPos"].cast<Vec3d>(),
        .camFocusPos = sceneDict["camFocusPos"].cast<Vec3d>(),
        .camFocusPointType = static_cast<CameraFocusPointType>(sceneDict["camFocusPointType"].cast<int>()),
        .sceneBbox = AABB(sceneDict["sceneBbox"].cast<py::dict>()["min"].cast<Vec3d>(), sceneDict["sceneBbox"].cast<py::dict>()["max"].cast<Vec3d>()),
        .timeStep = sceneDict["timeStep"].cast<double>(),
        .stepNum = sceneDict["stepNum"].cast<int>(),
        .forwardConvergenceThresh = sceneDict["forwardConvergenceThresh"].cast<double>(),
        .backwardConvergenceThresh = sceneDict["backwardConvergenceThresh"].cast<double>(),
        .name = sceneDict["name"].cast<std::string>(),
    };
...
PYBIND11_MODULE(diffcloth_py, m)
{
    ...
    py::class_<Simulation>(m, "Simulation")
        ...
        .def_readwrite("clothFrictionalCoeff", &Simulation::clothFrictionalCoeff)
